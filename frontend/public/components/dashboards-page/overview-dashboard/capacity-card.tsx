...
import { formatVectorStats, formatMatrixStats } from '../../graphs/utils';
...
const getLastStats = (response, formatStats): number => {
  const stats = formatStats(response);
  return stats.length > 0 ? stats[stats.length - 1].y as number : null;
};
    ...
    <DashboardCard>
      ...
      <DashboardCardBody>
        <CapacityBody>
          <CapacityItem
            title="CPU"
            used={getLastStats(cpuUtilization, formatMatrixStats)}
            total={100}
          ...
          <CapacityItem
            title="Memory"
            used={getLastStats(memoryUtilization, formatMatrixStats)}
            total={getLastStats(memoryTotal, formatVectorStats)}
            formatValue={humanizeBinaryBytesWithoutB}
            isLoading={!(memoryUtilization && memoryTotal)}
          />
          <CapacityItem
            title="Storage"
            used={getLastStats(storageUsed, formatMatrixStats)}
            total={getLastStats(storageTotal, formatVectorStats)}
            formatValue={humanizeBinaryBytesWithoutB}
            isLoading={!(storageUsed && storageTotal)}
          />
          <CapacityItem
            title="Network"
            used={getLastStats(networkUsed, formatVectorStats)}
            total={getLastStats(networkTotal, formatVectorStats)}
