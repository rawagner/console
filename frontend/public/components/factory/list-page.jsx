...
export const FireMan_ = connect(null, {filterList})(
  class ConnectedFireMan extends React.PureComponent {
    ...
    runOrNavigate = (itemName) => {
      const createProps = this.props.createProps || {};
      if (createProps.action && createProps.action(itemName)) {
        const action = createProps.action(itemName);
        action();
      } else if (createProps.createLink) {
