...
type VirtualizedTableProps<D = any, R = any> = {
  data: D[];
  loaded: boolean;
  loadError: any;
...
const VirtualizedTable = <D extends any, R extends any = {}>({
  data,
  loaded,
  loadError,
  columns: allColumns,
  ...
  data = React.useMemo(() => {
    ...
    if (!sortColumn.sort) {
      ...
      const sortFunc = (a: D, b: D) =>
        sortResourceByValue(a, b, sortBy.direction, (obj) =>
          _.get(obj, sortColumn.sort as string, ''),
        );
