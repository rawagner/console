...
const DefaultPage_: React.FC<DefaultPageProps> = ({ flags, activePerspective }) => {
  ...
  const perspective = perspectives.find((p) => p.properties.id === activePerspective);
...
const AppContents_: React.FC<AppContentsProps> = ({ activePerspective }) => {
  ...
  const pluginPageRoutes = React.useMemo(() => getPluginPageRoutes(activePerspective, routePages), [
    activePerspective,
    routePages,
  ]);
...
const connectToProps = (state: RootState) => ({
  activePerspective: getActivePerspective(state),
});

const AppContents = connect(connectToProps)(AppContents_);
