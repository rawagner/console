  ...
  class CatalogListPage extends React.Component<CatalogListPageProps, CatalogListPageState> {
    ...
    componentDidUpdate(prevProps) {
      ...
      if (
        ...
        !_.isEqual(devCatalogExtensions, prevProps.devCatalogExtensions)
      ) {
        const items = this.getItems();
        this.setState({ items });
      }
...
export const Catalog = connectToFlags<CatalogProps>(
  ...
  const plugins = useExtensions<DevCatalogModel>(isDevCatalogModel);
  const pluginResources = React.useMemo(
    () =>
      plugins.map(({ properties }) => ({
        isList: true,
