...
import { withExtensions, SecretHandleChangeExtenion, isSecretExtension } from '@console/plugin-sdk';
...
export const AddSecretToWorkloadModal = withExtensions<SecretExtensionProps>({
  ...
  > {
    ...
    handleChange: React.ReactEventHandler<HTMLInputElement> = (event) => {
      ...
      this.props.secretExtensions.forEach((s) => s.properties.handleChange(name, value));
      this.setState({
        [name]: value,
      } as any);
    };
...
type SecretExtensionProps = {
  secretExtensions: SecretHandleChangeExtenion[];
};
