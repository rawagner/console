...
export function getDefaultPerspective() {
  ...
  const perspectives = pluginStore.getAllExtensions().filter(isPerspective);
  if (activePerspective && !perspectives.some((p) => p.properties.id === activePerspective)) {
    // invalid saved perspective
    activePerspective = undefined;
  }
  if (!activePerspective) {
    // assign default perspective
    const defaultPerspective = perspectives.find((p) => p.properties.default);
