...
export const getK8sHealthState: HealthHandler<string> = (k8sHealth, error, resource) => {
  if (error) {
...
export const getControlPlaneComponentHealth: HealthHandler<PrometheusResponse> = (
  response,
  error,
) => {
  if (error) {
...
export const getControlPlaneHealth: HealthHandler<PrometheusResponse[]> = (
  responses = [],
  error: any[],
) => {
  const componentsHealth = responses.map((r, index) =>
    getControlPlaneComponentHealth(r, error[index]),
  );
