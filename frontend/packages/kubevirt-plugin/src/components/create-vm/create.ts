...
import { FormState } from './forms/create-vm-form-reducer';
import { BootSourceState } from './forms/boot-source-form-reducer';
import { DataSource } from './forms/data-source';
import { DataVolumeWrapper } from '../../k8s/wrapper/vm/data-volume-wrapper';
import { windowsToolsStorage } from '../create-vm-wizard/redux/initial-state/storage-tab-initial-state';
...
export const getRootDataVolume: GetRootDataVolume = ({
  ...
  const size = dataSource === DataSource.CLONE ? pvcSize : `${sizeValue}${sizeUnit}`;
  const dataVolume = new DataVolumeWrapper()
    ...
    .setAccessModes([accessMode || AccessMode.READ_WRITE_ONCE]);
  switch (dataSource) {
    case DataSource.URL: {
      dataVolume.setType(DataVolumeSourceType.HTTP, { url });
      break;
    }
    case DataSource.CLONE: {
      dataVolume.setType(DataVolumeSourceType.PVC, { name: pvcName, namespace: pvcNamespace });
      break;
    }
    case DataSource.REGISTRY: {
...
export const createVM = async (
  ...
  if (
    ...
    if (customSource?.dataSource?.value) {
      ...
      rootDataVolume = getRootDataVolume({
        name: rootVolume.getDataVolumeName(),
        accessMode: AccessMode.fromString(accessModes[0]),
        cdRom: isCDRom,
