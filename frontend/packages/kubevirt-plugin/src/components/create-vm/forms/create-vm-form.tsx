...
import { TEMPLATE_BASE_IMAGE_NAME_PARAMETER } from '../../../constants/vm';
...
export const CreateVMForm: React.FC<CreateVMFormProps> = ({
  ...
  const onNameChange = (value: string) => {
    dispatch({ type: FORM_ACTION_TYPE.SET_NAME, payload: value });
    const validation = validateVmLikeEntityName(value, namespace, vms, {
      existsErrorMessage: VIRTUAL_MACHINE_EXISTS,
      subject: 'Name',
    });
    dispatch({ type: FORM_ACTION_TYPE.NAME_VALIDATION, payload: validation });
  };

  const onNamespaceChange = (value: string) => {
    dispatch({ type: FORM_ACTION_TYPE.SET_NAMESPACE, payload: value });
    const validation = validateVmLikeEntityName(value, namespace, vms, {
      existsErrorMessage: VIRTUAL_MACHINE_EXISTS,
      subject: 'Name',
    });
  ...
  if (customSource?.dataSource) {
    ...
    switch (customSource.dataSource?.value) {
      case DataSource.URL:
