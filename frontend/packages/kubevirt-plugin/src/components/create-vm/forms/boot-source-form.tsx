...
export const BootSourceForm: React.FC<BootSourceFormProps> = ({ state, dispatch, withUpload }) => {
  ...
  const handleStorageClass = (scName: string) => {
    ...
    dispatch({ type: BOOT_ACTION_TYPE.SET_ACCESS_MODE, payload: AccessMode.READ_WRITE_ONCE });
    dispatch({
      type: BOOT_ACTION_TYPE.SET_ACCESS_MODES,
      payload: modes.map((m) => AccessMode.fromString(m)),
    });
    dispatch({
  ...
  return (
    <Form onSubmit={preventDefault}>
        ...
        <FormPFSelect
          ...
          onSelect={(e, value) =>
            dispatch({
              ...
              payload: value as DataSource,
            })
          }
          selections={state.dataSource?.value}
        >
          {DataSource.getAll()
            .filter((ds) => (withUpload ? true : ds !== DataSource.UPLOAD))
            .map((ds) => (
      ...
      {state.dataSource?.value === DataSource.URL && (
        <FormRow
          fieldId="form-ds-url"
          title="Import URL"
          isRequired
      ...
      {state.dataSource?.value === DataSource.REGISTRY && (
        <FormRow
      ...
      {state.dataSource?.value === DataSource.CLONE && (
        <>
          <FormRow
      ...
      {[DataSource.UPLOAD, DataSource.URL, DataSource.REGISTRY].includes(
      ...
      <ExpandableSection toggleText="Advanced">
        ...
        <FormRow fieldId="form-ds-access-mode" title="Access mode" isRequired>
          <FormSelect
            value={state.accessMode?.value}
            onChange={(value) =>
              dispatch({
                type: BOOT_ACTION_TYPE.SET_ACCESS_MODE,
                payload: AccessMode.fromString(value),
              })
            }
            id="vm-select-am"
            name="vm-select-am"
          ...
          >
            {state.accessModes?.value?.map((am) => (
              <FormSelectOption key={am.getValue()} value={am} label={am.toString()} />
            ))}
          </FormSelect>
          {!scLoaded && <LoadingInline />}
        </FormRow>
