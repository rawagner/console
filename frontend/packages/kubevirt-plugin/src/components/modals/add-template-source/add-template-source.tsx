...
import { DataSource } from '../../create-vm/forms/data-source';
import { getRootDataVolume } from '../../create-vm/create';
...
export const AddTemplateSourceModal: React.FC<ModalComponentProps &
  ...
  const onSubmit = async () => {
    ...
    const dvObj = getRootDataVolume({
      ...
      .asResource();
    try {
      if (dataSource?.value === DataSource.UPLOAD) {
          ...
          <ModalFooter errorMessage={error} inProgress={false}>
            <ActionGroup className="pf-c-form pf-c-form__actions--right pf-c-form__group--no-top-margin">
              ...
              <Button
                ...
                onClick={onSubmit}
              >
                {`Save and ${dataSource?.value?.getAction() ?? 'upload'}`}
              </Button>
            </ActionGroup>
          </ModalFooter>
        </>
